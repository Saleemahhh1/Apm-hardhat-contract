...
contract MarketFactory is Ownable {
    ...
    function createMarket(string memory _question, uint256 _deadline, uint256 _feeBps) external {
        // collect creation fee
        token.safeTransferFrom(msg.sender, address(this), CREATION_FEE);
        // add portion to rewardPool (e.g., 80%)
        uint256 poolShare = (CREATION_FEE * 80) / 100;
        rewardPool += poolShare;
        // remainder could be fee to treasury
        uint256 feeShare = CREATION_FEE - poolShare;
        if (feeShare > 0) token.safeTransfer(treasury, feeShare);

        // create market
        PredictionMarket market = new PredictionMarket(address(token), _question, _deadline, address(this), _feeBps == 0 ? defaultFeeBps : _feeBps, treasury);
